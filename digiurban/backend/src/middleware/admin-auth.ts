...
export const adminAuthMiddleware = async (
  ...
  try {
    ...
    // Verificar se o tenant do token corresponde ao tenant da requisição
    // NOTA: Com login inteligente, o tenant vem do token JWT, não da requisição
    // Apenas verificar se ambos existem E são diferentes (conflito real)
    if (req.tenant && decoded.tenantId && req.tenant.id !== decoded.tenantId) {
      console.log(`[AUTH DEBUG] Tenant mismatch: req.tenant=${req.tenant.id} vs decoded.tenantId=${decoded.tenantId}`);
      // PERMITIR ACESSO: o tenantId do token é a fonte da verdade
      // Apenas logar o mismatch mas não bloquear
    }
