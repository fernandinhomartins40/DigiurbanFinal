...
export async function recordFailedLogin(
  ...
  try {
    ...
    if (shouldLock) {
      ...
      await logAccountLocked(model, account.id, tenantId, newAttempts);
    }
...
export const accountLockoutMiddleware = (model: 'user' | 'citizen') => {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const { isLocked, remainingTime } = await checkAccountLockout(model, identifier, tenant.id);

      if (isLocked) {
        console.warn(
          `[SECURITY] Tentativa de login em conta bloqueada - ` +
          `Tipo: ${model}, Identifier: ${identifier}, Tenant: ${tenant.id}`
        );
