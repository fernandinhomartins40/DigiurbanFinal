...
router.get('/stats', authenticateToken, requireManager, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const [
    ...
    prisma.criticalPoint.count({ where: { tenantId, isActive: true } }),
    prisma.securityAlert.count({ where: { tenantId, isActive: true } }),
    prisma.securityPatrol.count({ where: { tenantId } }),
    prisma.municipalGuard.count({ where: { tenantId, isActive: true } }),
    prisma.surveillanceSystem.count({ where: { tenantId, isActive: true } }),
  ]);
...
router.get('/attendances', authenticateToken, requireManager, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const [data, total] = await Promise.all([
    ...
    prisma.securityAttendance.count({ where: where as any }),
  ]);
