    ...
    try {
      ...
      const [
        ...
        prisma.localBusiness.aggregate({
          where: { tenantId, isActive: true, isTourismPartner: true },
          _count: { id: true },
        }),
...
router.get(
  ...
  async (req, res) => {
    try {
      const authReq = req as AuthenticatedRequest;
      const tenantId = authReq.tenantId;
      const { page = 1, limit = 20, businessType, category } = req.query;

      const skip = (Number(page) - 1) * Number(limit);
      const where: any = { tenantId, isTourismPartner: true };
...
router.get(
  ...
  async (req, res) => {
    try {
      const authReq = req as AuthenticatedRequest;
      const tenantId = authReq.tenantId;
      const { page = 1, limit = 20, routeType, difficulty } = req.query;
