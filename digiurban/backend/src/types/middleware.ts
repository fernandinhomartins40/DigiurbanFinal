...
import { UserRole, TenantStatus, Tenant } from '@prisma/client';
import { Schema } from 'joi';
import { UserWithRelations } from './common';
import { TenantLimits, UsageStats } from './tenant';
...
export function isAuthenticated(req: Request): req is AuthenticatedRequest {
  ...
  return !!(extReq.userId && extReq.user && extReq.userRole && extReq.tenantId && extReq.tenant);
}
...
export function ensureAuthenticated(req: Request): asserts req is AuthenticatedRequest {
  const extReq = req as any;
  if (!extReq.userId || !extReq.user || !extReq.userRole || !extReq.tenantId || !extReq.tenant) {
...
/**
 * Request com informações de tenant
 */
export interface TenantRequest extends Omit<Request, 'tenant'> {
  tenantId?: string;
  tenant?: TenantWithMeta & { settings?: Record<string, unknown> };
}

/**
 * Request que passou por tenantMiddleware
 * Garante que tenant e tenantId estão definidos
 */
export interface GuaranteedTenantRequest extends Omit<Request, 'tenant'> {
  tenantId: string;
  tenant: TenantWithMeta & { settings?: Record<string, unknown> };
}

/**
 * Request autenticado com tenant
 */
export interface AuthenticatedTenantRequest extends Omit<Request, 'tenant' | 'user'> {
  userId: string;
  user: UserWithRelations;
  userRole: UserRole;
  tenantId: string;
