...
router.get('/stats', authenticateToken, requireManager, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!dept) {
    res.status(404).json({ error: 'Departamento de Planejamento Urbano não encontrado' });
    return;
  }
  ...
  const [
    ...
    prisma.projectApproval.count({ where: { tenantId } }),
    prisma.buildingPermit.count({ where: { tenantId } }),
    prisma.businessLicense.count({ where: { tenantId } }),
    prisma.certificateRequest.count({ where: { tenantId } }),
    prisma.urbanInfraction.count({ where: { tenantId } }),
    prisma.urbanZoning.count({ where: { tenantId } }),
    prisma.urbanPlanningAttendance.count({ where: { tenantId } }),
  ]);
