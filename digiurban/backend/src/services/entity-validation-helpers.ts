...
/**
 * Valida se um relacionamento existe e pertence ao tenant correto
 */
export async function validateTenantRelation<T>(
  ...
  if (!record) {
    throw new Error(`${modelName} não encontrado ou não pertence a este tenant`);
  }
...
/**
 * Helper para validar múltiplos relacionamentos com tenant
 */
export async function validateMultipleRelations(
  ...
  for (const relation of relations) {
    if (relation.id) {
      await validateTenantRelation(tx, relation.modelName, relation.id, tenantId);
    }
  }
}
 ...
 *
 * @example
 * const patient = await validateTenant<Patient>(tx, 'patient', patientId, tenantId);
 */
export async function validateTenant<T>(
  ...
  return validateTenantRelation<T>(tx, model, id, tenantId);
}
