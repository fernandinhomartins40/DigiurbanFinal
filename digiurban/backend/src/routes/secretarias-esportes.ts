  ...
  async (req, res) => {
    try {
      ...
      const [data, total, activeCount] = await Promise.all([
        ...
        prisma.athlete.count({ where: { tenantId, isActive: true } }),
      ]);
  ...
  async (req, res) => {
    try {
      ...
      const [data, total, activeCount] = await Promise.all([
        ...
        prisma.sportsTeam.count({ where: { tenantId, isActive: true } }),
      ]);
...
router.get(
  ...
  async (req, res) => {
    try {
      const authReq = req as AuthenticatedRequest;
      const tenantId = authReq.tenantId;
      const { page = 1, limit = 20, sport, search } = req.query;
      ...
      const [data, total, activeCount] = await Promise.all([
        ...
        prisma.sportsSchool.count({ where: { tenantId, isActive: true } }),
      ]);
