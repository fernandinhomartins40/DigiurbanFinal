...
import { ModuleExecutionContext, ModuleExecutionResult } from '../../module-handler';
...
export class InfrastructureProblemHandler {
  ...
  static async getStats(tenantId: string) {
    const [total, pending, inProgress, resolved, byType, byPriority] = await Promise.all([
      prisma.infrastructureProblem.count({ where: { tenantId } }),
      prisma.infrastructureProblem.count({ where: { tenantId, status: 'pending' } }),
      prisma.infrastructureProblem.count({ where: { tenantId, status: 'in_progress' } }),
      prisma.infrastructureProblem.count({ where: { tenantId, status: 'resolved' } }),
