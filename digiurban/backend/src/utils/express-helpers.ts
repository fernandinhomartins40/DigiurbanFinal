...
export function logRequest(req: Request, level: 'info' | 'warn' | 'error' = 'info'): void {
  const logData = {
    ...
    tenantId: (req as AuthenticatedRequest).tenantId || null,
  };
...
export function createRateLimiter(
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    const key = `${req.ip}:${(req as AuthenticatedRequest).tenantId || 'anonymous'}`;
    const now = Date.now();
...
export function createCacheMiddleware(ttlSeconds: number = 300) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const key = `${req.url}:${(req as AuthenticatedRequest).tenantId}`;
...
// Helper para criar middleware de validação de tenant
export function validateTenantAccess() {
  return asyncHandler<TenantRequest & AuthenticatedRequest>(async (req, res, next) => {
    if (!req.tenant || !req.user) {
      return sendError(res, 'Access denied', 'Acesso negado', 403);
    }

    // Verifica se o usuário pertence ao tenant
    if (req.user.tenantId !== req.tenant.id) {
      return sendError(res, 'Tenant mismatch', 'Usuário não pertence a este tenant', 403);
    }
