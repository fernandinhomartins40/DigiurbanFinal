...
// Handler com tenant - requer tenantId e tenant
export type TenantHandler = (
  req: Request & Required<Pick<Request, 'tenantId' | 'tenant'>>,
  res: Response,
  next: NextFunction
) => void | Promise<void>;

// Handler completo - requer autenticação e tenant
export type AuthenticatedHandler = (
  req: Request & Required<Pick<Request, 'userId' | 'user' | 'tenantId' | 'tenant'>>,
  res: Response,
  next: NextFunction
) => void | Promise<void>;
...
export type RoleHandler<T extends UserRole> = (
  req: Request &
    Required<Pick<Request, 'userId' | 'user' | 'tenantId' | 'tenant'>> & {
...
// Helper para criar handlers com tenant
export const createTenantHandler = (handler: TenantHandler): BaseHandler => {
  return createHandler(handler);
};

// Helper para criar handlers autenticados com tenant
